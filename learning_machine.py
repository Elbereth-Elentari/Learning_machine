...
with open("fiszki.csv", "r") as fiszki:
    fiszki = pd.read_csv(fiszki, header=None)
...
with open("fiszki.csv", "w") as nowe_fiszki, open("answered_questions.csv", "a+") as answered:
    fiszki.loc[fiszki[2] < 5].to_csv(nowe_fiszki, header=False, index=False)
    fiszki.loc[fiszki[2] == 5].to_csv(answered, header=False, index=False)
...
for level in range(0, 5):
    print("Remaining questions at level", level, ":", len(fiszki.loc[fiszki[2] == level]), "/", max_definitions[level])
